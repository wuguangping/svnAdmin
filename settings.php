...
if (check_request_var("save"))
{
    ...
    $cfgEngine->setValue("Repositories:svnclient", "SVNServeFile", $pSVNServeFile);
	...
	$cfgEngine->setValue("Repositories:svnclient", "SvnServeExecutable", $pSvnServeExecutable);
...
if (check_request_var("dotest") && check_request_var("dotestsec"))
{
	...
	switch($testSection)
	{
        ...
        case "SVNServeFile":
            if (file_exists($pSVNServeFile))
                if (is_writable($pSVNServeFile))
                    $msgOk = $appTR->tr("Test passed.");
                else
                    $msgErr = $appTR->tr("The folder exists but is not writable.");
            else
		...
		case "SvnServeExecutable":
			if (file_exists($pSvnServeExecutable))
				if (is_executable($pSvnServeExecutable))
					$msgOk = $appTR->tr("Test passed.");
				else
					$msgErr = $appTR->tr("The file exists but is not executable.");
			else
		...
		case "SvnServeExecutableRestart":
			if (file_exists($pSvnServeExecutable))
				if (is_executable($pSvnServeExecutable)) {
                    // 重启服务
                    global $appEngine;
                    $svnParentPath = $appEngine->getConfig()->getValue("Repositories:svnclient","SVNParentPath");
                    $svnServeFile = $appEngine->getConfig()->getValue("Repositories:svnclient","SVNServeFile");
                    exec("killall -9 svnserve");
                    exec("svnserve -d -r " . $svnParentPath . " --config-file=" . $svnServeFile);

                    $msgOk = $appTR->tr("Restart successed.");
                }
				else
					$msgErr = $appTR->tr("The file exists but is not executable.");
			else
...
$svnServeFile = $cfgEngine->getValue("Repositories:svnclient","SVNServeFile");
$svnServeFileEx = $cfgEngine->getValue("Repositories:svnclient","SVNServeFile");
...
$svnServeExecutable = $cfgEngine->getValue("Repositories:svnclient","SvnServeExecutable");
$svnServeExecutableEx = $cfgTpl->getValue("Repositories:svnclient","SvnServeExecutable");
SetValue("SVNParentPath", $svnParentPath);
SetValue("SVNParentPathEx", $svnParentPathEx);
SetValue("SVNServeFile", $svnServeFile);
SetValue("SVNServeFileEx", $svnServeFileEx);
...
SetValue("SvnServeExecutable", $svnServeExecutable);
SetValue("SvnServeExecutableEx", $svnServeExecutableEx);
